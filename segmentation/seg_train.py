...
from models.model_tools import load_model, save_model
from models.dice_score import dice_loss
...
def get_args():
   ...
   # Directories and file paths
    parser.add_argument('--data_dir', type=str, default="data/VS/T1", help='Path to the data domain directory')
    parser.add_argument('--save_dir', type=str, default="checkpoints/T1_seed_3", help='Path to save checkpoints')
    parser.add_argument('--checkpoints', type=str, default="", help='Path to saved checkpoints')
    ...
    parser.add_argument('--save_fre', type=int, default=40, help='Frequency of saving checkpoints')
...
if __name__ == '__main__':
    ...
    # 2. Create dataset
    train_ds = VSDataset(data_dir=args.data_dir, split='training', resolution=args.inp_size)
    val_ds = VSDataset(data_dir=args.data_dir, split='validation', resolution=args.inp_size)
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=1, drop_last=False)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=True, num_workers=1, drop_last=False)
